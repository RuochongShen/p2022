...
def peak_signal_noise_ratio(image_true, image_test, *, data_range=None):
    ...
    if data_range is None:
        ...
        if true_max > dmax or true_min < dmin:
            ...
            image_true[image_true < dmin] = dmin
            image_true[image_true > dmax] = dmax
            true_min, true_max = np.min(image_true), np.max(image_true)
        if true_min >= 0:
...
def calc_met_rsp(im1, im2, roi=None, in_matlab=True):
    """
    keep in line with MATLAB (type: double)
    may need to normalize the images to [0, 1]
    """
    pixeln = np.size(im1)
    if roi:
        top, bottom, left, right = roi
        im1, im2 = im1[top:bottom, left:right], im2[top:bottom, left:right]
        pixeln = (bottom - top) * (right - left)

    rmse = np.sqrt(mean_squared_error(im1, im2) * pixeln)
    if in_matlab:
        psnr = peak_signal_noise_ratio(im1, im2, data_range=1)
        ssim = structural_similarity(im1, im2, data_range=1, gaussian_weights=True)
    else:
